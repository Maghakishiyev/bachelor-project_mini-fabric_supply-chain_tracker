...
const CITIES = [
  "Warsaw", "Berlin", "Paris", "Madrid", "Rome", "London", "Amsterdam", 
  "Vienna", "Brussels", "Copenhagen", "Helsinki", "Oslo", "Stockholm", 
  "Zurich", "Prague", "Budapest", "Athens", "Dublin", "Lisbon", "Bucharest"
];

// Use the actual status values from the system
const STATUSES = [
  "CREATED", "PICKED_UP", "IN_TRANSIT", "DELIVERED", "EXCEPTION"
];
...
const peerEndpoint = process.env.PEER_ENDPOINT || "peer0.manufacturer.example.com:7051";
...
(async () => {
  ...
  const timer = setInterval(async () => {
    ...
    if (createdShipments.length > 10 && Math.random() > 0.6) {
      ...
      id = createdShipments[Math.floor(Math.random() * createdShipments.length)];
      ...
      await cc.submitTransaction("CreateShipment", id, origin, destination);
    ...
    console.log(JSON.stringify({ id, type: transactionType, latency, ts: t0 }));
    sent++;
  }, 1000 / RATE);
})();